...
public class SeanSinnott
{
	...
	public static void obtainSalesReportForGenre(ArrayList<GameDetail> games, ArrayList<GameSale> sales, ArrayList<GameGenre> genres)
	{
		/*GameSale aSale;
		GameDetail aDetail;
		String genre = "";
		...
		String[] result;
		String gameGenre = "";
		int genreID, unitsSold, date, gameID;
		boolean found = false;
		String lineFromGameDetails = "";
		FileReader reader = new FileReader("GameDetails.txt");
		Scanner scanner = new Scanner(reader);
		while (scanner.hasnext())
		{	
			lineFromGameDetails = scanner.nextLine();
			results = lineFromGameDetails.split(",");
			gameGenre = Integer.parseInt(result[3]);
		}
		scanner.close();
		reader.close();
		String lineFromGamesSales = "";
		FileReader secondReader = new FileReader("GamesSales.txt");
		Scanner secondScanner = new Scanner(secondReader);
		while (secondScanner.hasnext())
		{	
			lineFromGameDetails = secondScanner.nextLine();
			results = lineFromGamesSales.split(",");
			date = Integer.parseInt(result[0]);
			unitsSold = Integer.parseInt(result[1]);
		}
		System.out.println(gameGenre + "," + date + "," + unitsSold);*/
	}
